...
export async function onRequestGet(context) {
  ...
  const aggregateKey = new Request(`${url.origin}/__edge/pulse/asia-memory-snapshot/live`);
  ...
  try {
    ...
    const [tsmc, kioxia] = await Promise.all([
      fetchJson(`${url.origin}/api/pulse/tsmc-brief`),
      fetchJson(`${url.origin}/api/pulse/kioxia-brief`),
    ]);
    ...
    const payload = {
      ...
      tsmc: tsmc.quote || null,
      kioxia: kioxia.quote || null,
    };
