...
def main():
    ...
    final_status.update({
        ...
        'etaText': '0.0s',
        'currentPart': None,
        'final': str(workdir / 'final_raw_dump.txt'),
        'raws': raws,
    })
    write_status(status_file, final_status)
    print(json.dumps({'final': str(workdir / 'final_raw_dump.txt'), 'status': str(status_file), 'parts': len(parts), 'raws': raws}, ensure_ascii=False))
